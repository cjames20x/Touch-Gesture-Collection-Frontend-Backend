...
import { submitGestures, authenticate, type SequencePayload, type GesturePayload, type TouchEventPayload } from './api.js';
 ...
 *   };
 */
async function onTrainingComplete(completedSequences: SequencePayload[]) {
  ...
  try {
    ...
    if (statusEl) statusEl.textContent = `❌ Backend error: ${(err as Error).message}`;
    console.error(err);
  }
}
 ...
 */
async function onEvalComplete(evalSequence: SequencePayload) {
  const { participantId = 'unknown' } = getUser();
  const sessionId = getSessionId();
  ...
  try {
    ...
    // Show "Done" button
    const doneBtn = document.getElementById('done-btn') as HTMLAnchorElement | null;
